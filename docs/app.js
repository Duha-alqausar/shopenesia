...
function loadCart(open=false){
  ...
  document.getElementById('cartItems').innerHTML = cart.length ? cart.map(i => {
    ...
    return `<div class="cart-item"><div>${p.image}</div><div><strong>${p.name}</strong><br><small>${i.qty} × ${rupiah(p.price)}</small></div><button class="secondary" onclick="removeItem('${p.id}')">Hapus</button></div>`;
